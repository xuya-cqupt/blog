...
Base.prototype.animate = function(obj) {
  for (var i = 0; i < this.elements.length; i++) {
    ...
    var attr = obj['attr'] == 'x' ? 'left' : obj['attr'] == 'y' ? 'top' :
      ...
      obj['attr'] == 'o' ? 'opacity' :
      obj['attr'] == undefined ? 'left' : obj['attr'];

    var start = obj['start'] != undefined ? obj['start']:
         attr=='opacity'  ?  parseFloat(getStyle(elem,attr))*100:
    ...
    var mul = obj['mul'];
      if(alter != undefined&&target == undefined){
        target = alter+start;
            ...
            throw new Error('alter增量或target目标量或mul对象必须传一个！');
       }
       
   if(start > target) step = -step;
   //根据start值进行初始化
   if(attr == 'opacity'){
    elem.style.opacity = parseInt(start) / 100;
    elem.style.filter = 'alpha(opacity='+parseInt(start)+')';
   }else{
...
clearInterval(elem.timer);
    elem.timer = setInterval(function() {
        var flag = true;
    for(var i in mul){
       attr = i  == 'w'? 'width' :i == 'o' ? 'opacity' :
            i  == 'y'?'top':i == 'h'?'height':i == 'x'?'left':i!=undefined ? i:'left';
     target = mul[i];
     if(type == 'buffer'){
      step = attr =='opacity' ? (target-parseFloat(getStyle(elem,attr))*100)/speed:
                    (target - parseInt(getStyle(elem,attr))) /speed;
      ...
      if(attr=='opacity'){   
           if(step == 0){
              setOpacity();
           }else if (step > 0 && Math.abs(target -parseFloat(getStyle(elem,attr))*100) <= step) {
              setOpacity();
          } else if (step < 0 && (parseFloat(getStyle(elem,attr)) * 100 - target)<= Math.abs(step)) {
            setOpacity();
          } else {
             var temp = parseFloat(getStyle(elem,attr))*100;
             elem.style.opacity =parseInt(temp + step)/100;
             elem.style.filter = 'alpha(opacity='+parseInt(temp+step)+')';
         }
       if(parseInt(target)!=parseInt(parseFloat(getStyle(elem,attr))*100)) flag = false;

      }else{
           if(step==0){
              setTarget();
          }else if (step > 0 && Math.abs(target - parseInt(getStyle(elem, attr) )<=step)) {
              setTarget();
          } else if (step < 0 && (parseInt(getStyle(elem, attr))-target)<= Math.abs(step)) {
              setTarget();
          } else {
            elem.style[attr] = parseInt(getStyle(elem, attr) )+ step + 'px';
          }
          if(parseInt(target)!=parseInt(getStyle(elem,attr)))  flag = false;
        }//else  end
  ...
  function setOpacity(){
    elem.style.opacity = parseInt(target)/100;
    elem.style.filter = 'alpha(opacity='+parseInt(target)+")";
  }
  function setTarget() {
